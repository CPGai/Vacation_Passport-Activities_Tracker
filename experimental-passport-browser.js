(function(){
  ...
  function createExperimentalDocument(count = 14) {
    ...
    return {
      ...
      pages: Array.from({ length: actualCount }, (_, i) => {
        const n = i + 1;
        const isCover = n === 1;
        const isIdentity = n === 2;
        const hasBackCovers = actualCount >= 14;
        const isBackCoverInside = hasBackCovers && n === actualCount - 1;
        const isBackCover = hasBackCovers && n === actualCount;
        ...
        return {
          ...
          material,
          backgroundColor,
          activity: kind === 'activity' ? { title: `Actividad ${n - 1}`, description: 'Escribe aquí la descripción.' } : null,
          frameColor: '#9fcbd5',
          showNumber: true,
  ...
  function addPage(doc, template = 'activity') {
    ...
    doc.pages.splice(insertIndex, 0, p);
  ...
  function getTargetPagesByScope(doc, scope = 'single', selectedPageNumber = 1, customSelection = []) {
  ...
  function applyPageSettings(targetPages, settings = {}) {
  ...
  function applyTemplate(p, t) {
    p.template = t;
    p.kind = t === 'cover' ? 'cover' : t === 'identity' ? 'identity' : t === 'activity' ? 'activity' : 'blank';
    if (p.kind !== 'activity') p.activity = null;
    if (p.kind === 'activity' && !p.activity) p.activity = { title: 'Nueva actividad', description: 'Escribe aquí la descripción.' };
    return p;
  }

  function reorderActivities(doc, numbers) {
    const pages = doc.pages.filter(p => p.number >= 3);
    if (numbers.length !== pages.length || numbers.some(n => Number(n) < 3) || new Set(numbers.map(Number)).size !== pages.length) {
      throw new Error('El orden debe contener todas las actividades.');
    }
    const snapshots = numbers.map(n => clone(doc.pages.find(p => p.number === Number(n))));
    pages.forEach((p, i) => {
      const snap = snapshots[i];
      p.activity = snap.activity;
      p.image = snap.image;
      p.stamp = snap.stamp;
      p.fontSize = snap.fontSize || fontSize();
      p.textOpacity = snap.textOpacity || textOpacity();
      p.textPosition = snap.textPosition || textPosition();
      p.numberPosition = snap.numberPosition || 'top-center';
      p.template = snap.template || 'activity';
      p.kind = snap.kind || 'activity';
      p.backgroundColor = snap.backgroundColor || '#edf8fa';
    });
    return numbers.map(Number);
  }

  function removePage(doc, number) {
    ...
    const index = doc.pages.findIndex(p => p.number === Number(number));
    if (index < 0) throw new Error('Página inválida.');
    doc.pages.splice(index, 1);
    doc.pages.forEach((p, i) => { p.number = i + 1; });
    doc.pageCount = doc.pages.length;
    doc.customImposition = null;
    return doc;
  }

  function calculateMaterialImposition(doc, overrideMode) {
  ...
  function getEffectiveImposition(doc) {
    if (doc.customImposition) return doc.customImposition;
    return calculateMaterialImposition(doc);
  }

  function setCustomPanelPage(doc, section, sideIndex, panelIndex, pageNumber) {
  ...
  function swapSheetPanels(doc, fromLoc, toLoc) {
    if (!doc.customImposition) {
      doc.customImposition = clone(calculateMaterialImposition(doc));
    }
  ...
  function resetCustomImposition(doc) {
    doc.customImposition = null;
    return calculateMaterialImposition(doc);
  }
  ...
  function parseActivitiesCsv(csvString) {
  ...
  function normalizeCsvImagePath(rawPath) {
  ...
  function importActivitiesFromCsv(doc, csvString) {
    ...
    parsedActivities.forEach((item) => {
      ...
      if (item.image) {
        const normalizedImg = normalizeCsvImagePath(item.image);
        page.image = {
          src: normalizedImg,
          file: normalizedImg.split(/[/\\]/).pop(),
          rule: 'manual',
          scale: 90,
          x: 0,
          y: 0,
